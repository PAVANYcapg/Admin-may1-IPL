...
class BidderTest {
	...
	void testUpdate() {
		...
		bidder4.setName("simha");
		bidder4.setEmail("simha8096@gmail.com");
		bidder4.setPassword("12345");
		bidder4.setPhoneNo("0987456321");
		bidderService.add(bidder4);
		bidder4.setBidderId(2);
		bidderService.update(bidder4);
	
		Assert.assertEquals(true,bidderService.update(bidder4));
		}

	@Test
	void testDelete() {
		Bidder bidder6=bidderService.findTeam(3);
		bidderService.delete(bidder6.getBidderId());
		Bidder bidder7 = bidderService.findTeam(3);
